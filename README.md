...
* 28057 XML files of articles citing retracted articles, according to the file above, are downloaded from PubMedCentral (using the script `getPubmedCorpus.py` with option `downloadPubmedFiles` set to true)
  ...
  * among those 3196 contexts, we manually deleted some contexts which seemed incomplete (and therefore, difficult to analyze), starting from the beginning of the file above, until we reached a total of 2500 contexts.